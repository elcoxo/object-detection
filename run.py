...
if __name__== "__main__":
    ...
    if (cap.isOpened() == True):
        ...
        fps = cap.get(5)
        print('Weight x h FPS: ', width, 'x', height, fps)

        fourcc = cv2.VideoWriter_fourcc(*'MJPG')
        video = cv2.VideoWriter(os.path.basename(filename).split(".")[0] + '_output.avi', fourcc, fps, (width,height))
    else:
    ...
    while(cap.isOpened()):
        ...
        if flag==True:
            ...
            output = postprocess(img, cap_blob)
            video.write(output)
        else:
            break
